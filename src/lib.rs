...
pub const V_MAX: f64 = 3.0; //gm6020_can::V_MAX;
...
*/
#[no_mangle]
pub extern "C" fn init(interface: *const c_char) -> *const Gm6020Can {
    let inter: &str;
    if interface.is_null() {
        println!("Invalid c-string received for interface name (null pointer)");
        return null::<Arc<Gm6020Can>>() as *const Gm6020Can;
    }
    else {
        unsafe {
    ...
    gm6020_can::init(inter).map_or_else(|e| {eprintln!("{}", e); null::<Gm6020Can>() as *const Gm6020Can}, |v| Arc::into_raw(v))
}
...
*/
#[no_mangle]
pub extern "C" fn cleanup(gm6020_can: *const Gm6020Can, period_ms: u64){
    if gm6020_can.is_null(){
        println!("Invalid handle (null pointer)");
        return;
    }
    let handle: Arc<Gm6020Can> = unsafe {Arc::from_raw(gm6020_can)};
    gm6020_can::cleanup(handle.clone(), period_ms);
}
...
*/
#[no_mangle]
pub extern "C" fn run_once(gm6020_can: *const Gm6020Can) -> i8{
    if gm6020_can.is_null(){
        println!("Invalid handle (null pointer)");
        return -1;
    }
    let handle: Arc<Gm6020Can> = unsafe{Arc::from_raw(gm6020_can)}; // Wrap the raw pointer into Rust object
...
*/
#[no_mangle]
pub extern "C" fn set_cmd(gm6020_can: *const Gm6020Can, id: u8, mode: CmdMode, cmd: f64) -> i8{
    if gm6020_can.is_null(){
        println!("Invalid handle (null pointer)");
        return -1;
    }
...
*/
#[no_mangle]
pub extern "C" fn get_state(gm6020_can: *const Gm6020Can, id: u8, field: FbField) -> f64{
    if gm6020_can.is_null(){
        println!("Invalid handle (null pointer)");
        return f64::NAN;
    }
